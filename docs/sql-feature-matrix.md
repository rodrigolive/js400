...
| `extendedDynamic` / `packageCache` / `sqlPackage` / `packageLibrary` / `packageError` | Supported (runtime, pending live qualification) | `extendedDynamic=true` + a `sqlPackage` name enables real JTOpen `JDPackageManager` behavior. On first prepare or first `executeImmediate`, js400 sends `FUNCTIONID_CREATE_PACKAGE` (`0x180F`) on a dedicated connection-scoped RPB with `PACKAGE_NAME` (`0x3804`) + `LIBRARY_NAME` (`0x3801`); subsequent packageable prepares emit both codepoints + `prepareOption=1` on `PREPARE_AND_DESCRIBE` / `EXECUTE_IMMEDIATE`, unpackageable prepares emit the empty `PACKAGE_NAME` codepoint (length-only) the way JTOpen does. Package name normalization is byte-for-byte JTOpen — up to 6 chars + 4-char suffix over `commitMode`, `dateFormat`, `dateSeparator`, `timeFormat`, `timeSeparator`, `decimalSeparator`, `naming`, `translateHex`, including the `commitMode=4`/*RR* dateSep remap. `packageError` honors `none` / `warning` / `exception` with the JTOpen contract — `warning` queues a `SqlWarning` for `Connection.getWarnings()` via `DbConnection.drainPackageWarning()`. `packageCache=true` additionally fires `FUNCTIONID_RETURN_PACKAGE` (`0x1815`) after create and caches the raw `DBReplyPackageInfo` blob. The cache-hit **skip-prepare** path (decoding the blob and reusing a packaged statement name) is deferred pending live-host qualification; until it lands, `packageHits` stays at 0. Counters: `packageCreates`, `packageFetches`, `packageHits` on `StatementManager.metrics`. Knobs-off path is unchanged on the wire. See [SQL Packages](sql-packages.md) for the full design and the `examples/sql-packages.js` walkthrough. |
