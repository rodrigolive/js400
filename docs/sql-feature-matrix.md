...
| Feature | Status | API |
| --- | --- | --- |
| `Statement.setQueryTimeout(n)` | Supported (post-RTT) | Positive `n` arms a per-execute `setTimeout(n * 1000)` that flips `isCancelled()`. After the in-flight RTT returns the wrapper throws `SqlError(HY008)` with the message "Query timeout exceeded". `n = 0` keeps the fast path: one boolean check, no timer, no allocation. The watchdog does NOT preempt an in-flight RTT — single-connection architecture has no side channel to inject `FUNCTIONID_CANCEL` mid-flight; that needs a separate connection (JTOpen `AS400JDBCConnectionImpl.cancel`) and is staged. |
| `Statement.cancel()` | Supported (client-side) | Flips `isCancelled()`. The next operation throws `SqlError(HY008)` "Statement was cancelled" and clears the flag so subsequent calls succeed. No wire-level cancel; cancel does NOT interrupt an in-flight execute (see staged note above). Also wired on `PreparedStatement` and applied to every execute path: `execute`, `executeForStream`, `executeCall`, `executeBatch`, generated-keys wrap, and the `Statement` ad-hoc immediate / batch paths. |
...
| `extendedDynamic` / `packageCache` / `sqlPackage` / `packageLibrary` | Plumbing-only (engine-visible) | Threaded from `DataSource` → `connect()` → `DbConnection` → `StatementManager` as opt-in fields; default behavior is unchanged when unset. The engine exposes `extendedDynamic` / `packageCache` / `packageName` / `packageLibrary` for inspection and three new counters (`packageHits`, `packageCreates`, `packageFetches`) on `StatementManager.metrics`. Counters stay at 0 until the next pass actually wires server-side SQL package interactions. The bench prints the package counters only when one of them is non-zero — keeps default output unchanged on the fast path. |
